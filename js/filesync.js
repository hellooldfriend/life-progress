/* ============================================================
   ...
   Перенесено из проекта sprint-progress: те же четыре точки опоры
   в Store (get / onChange / parseImport / replaceState).
   ============================================================ */
const FileSync = (() => {
  'use strict';
  ...
  const WRITE_DELAY = 500;          // не дёргаем диск на каждую отмеченную галочку
   ...
   *
   * @param {(existing: object) => Promise<boolean>|boolean} confirmLoad вернуть true, чтобы загрузить из файла
   * @returns {Promise<boolean>} подключились ли
   */
  async function connect(confirmLoad) {
    if (!supported()) return false;
    try {
      handle = await window.showSaveFilePicker({
        suggestedName: 'doozy.json',
        types: [{ description: I18N.t('file.description'), accept: { 'application/json': ['.json'] } }],
      });
    } catch (err) {
